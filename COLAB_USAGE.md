...
### 2️⃣ 재시작 후 실행
재시작이 완료되면 **새로운 셀**에서 다음 중 하나를 실행:
...
### `quick_setup()` - 더 간단한 사용법
```python
helper.quick_setup()  # 첫 번째 실행
# 재시작 후...
helper.quick_setup()  # 두 번째 실행
```

### `colab_setup_with_restart()` - 단계별 설정
```python
helper.colab_setup_with_restart()  # Phase 1: 폰트 설치
# 재시작 후...
exec(open('colab_restart_phase2.py').read())  # Phase 2: 설정 완료
```
...
### Q: 자동 실행 파일이 없어요
A: 첫 번째 실행 시 `auto_restart_setup.py` 파일이 생성됩니다.
